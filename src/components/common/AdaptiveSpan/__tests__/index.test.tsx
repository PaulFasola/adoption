...
describe('AdaptiveSpan Component', () => {
  ...
  it('should have a title when showTitle is true', () => {
    const wrapper = shallow(<AdaptiveSpan showTitle text='Foobar' />);
  ...
  it('should apply general styling correctly', () => {
    const boldSpan = shallow(<AdaptiveSpan showTitle text='Foobar' style='bold' />);

    expect(toJson(boldSpan)).toMatchSnapshot();

    const wrapper = shallow(<AdaptiveSpan showTitle text='Foobar' style='italic' />);
  ...
  it('should replace a tag accordingly', () => {
    const wrapper = shallow(
      <AdaptiveSpan showTitle text='Foo{test}' mapping={[{ tag: '{test}', value: 'bar' }]} />
    );
  ...
  it('should replace occurencies matching tags defined in mapping property correctly', () => {
    ...
    expect(wrapper.html()).toEqual(
      '<span title="foo is not bar, but foo is foo">foo is not bar, but foo is foo</span>'
    );
  });
  ...
  it('should replace occurencies and apply the requested styles', () => {
    ...
    expect(wrapper.html()).toEqual(
      '<span title="foo is not bar, and bar is not foobar."><b>foo</b> is not <i>bar</i>, and <i>bar</i> is not foobar.</span>'
    );
  });
});
